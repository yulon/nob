...
namespace nob {
	void main();
	void add_frame_task(const std::function<void()> &handler, int duration = -1);
	void wait(size_t ms);
	...
	namespace world {
		inline float get_ground_height(const coords &cods) {
			shv::STREAMING::LOAD_SCENE(cods.x, cods.y, cods.z);

			float height;
			do {
				wait(1000);
				shv::GAMEPLAY::GET_GROUND_Z_FOR_3D_COORD(cods.x, cods.y, cods.z, &height, 0);
			} while (height == 0);
	...
	class model {
		public:
			...
			model(std::string name) : shv_model(shv::GAMEPLAY::GET_HASH_KEY((char *)name.c_str())) {
				if (shv::STREAMING::IS_MODEL_IN_CDIMAGE(shv_model) && shv::STREAMING::IS_MODEL_A_VEHICLE(shv_model)) {
			...
			/*static const size_t characters_sum = 696;

			static constexpr const char *const characters[696] {
			...
			static constexpr const char *const characters_description[696] {
			...
			static const size_t vehicles_sum = 400;

			static constexpr const char *const vehicles[400] {
	...
	class character {
		public:
			...
			coords pos(const coords &rcs_offset = {0, 0, 0}) {
				auto p = shv::ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(shv_ped, rcs_offset.x, rcs_offset.y, rcs_offset.z);
				return {p.x, p.y, p.z};
			}

			void move(const coords &pos) {
				shv::ENTITY::SET_ENTITY_COORDS(shv_ped, pos.x, pos.y, pos.z, TRUE, TRUE, TRUE, TRUE);
			}

			void place_on_ground() {
				auto c_pos = pos();
				shv::ENTITY::SET_ENTITY_COORDS(shv_ped, c_pos.x, c_pos.y, world::get_ground_height(c_pos) + 1, TRUE, TRUE, TRUE, TRUE);
			}
	};
	...
	namespace player {
		...
		inline void swicth(character chr) {
			...
			auto old_chr_cods = old_chr.pos({ 0, 0, 0 });
			auto chr_cods = chr.pos({ 0, 0, 0 });

			int st = shv::STREAMING::GET_IDEAL_PLAYER_SWITCH_TYPE(old_chr_cods.x, old_chr_cods.y, old_chr_cods.z, chr_cods.x, chr_cods.y, chr_cods.z);
	...
	class vehicle {
		public:
			...
			vehicle(model m, const coords &pos) : shv_vehicle(shv::VEHICLE::CREATE_VEHICLE(m.shv_model, pos.x, pos.y, pos.z, 0.0, FALSE, TRUE)) {}
