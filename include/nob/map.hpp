...
public:
	...
	static void remove_all_from_graphic(int g) {
		for (auto b = ntv::UI::GET_FIRST_BLIP_INFO_ID(g); b;
			 b = ntv::UI::GET_NEXT_BLIP_INFO_ID(g)) {
			ntv::UI::REMOVE_BLIP(&b);
		}
	}

	static void force_remove(bool toggle = true) {
		ntv::GAMEPLAY::SET_THIS_SCRIPT_CAN_REMOVE_BLIPS_CREATED_BY_ANY_SCRIPT(
			toggle);
	}
	...
	marker(const vector3 &pos) :
		_h(ntv::UI::ADD_BLIP_FOR_COORD(pos.x, pos.y, pos.z)) {
		if (_h) {
			auto h = _h;
	...
	marker(const vector3 &pos, float radius) :
		_h(ntv::UI::ADD_BLIP_FOR_RADIUS(pos.x, pos.y, pos.z, radius)) {
		if (_h) {
			auto h = _h;
			gc::delegate(*this, [h]() mutable {
				if (ntv::UI::DOES_BLIP_EXIST(h)) {
