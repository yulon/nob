...
// Reference from
// https://github.com/GTA-Lion/citizenmp/blob/master/components/rage-scripting-five/
...
public:
	enum class state_t : uint32_t { idle, running, killed, unk1, unk2 };
	...
	static script_thread_t *&current() {
		return *reinterpret_cast<script_thread_t **>(
			*(uintptr_t *)__readgsqword(88) + tls_off);
	}
	...
	// known: set 'context.state' to 'state_t::killed', and dealloc
	// 'script_context'.
	...
	RUA_SASSERT(sizeof(context_t) == 168);
	...
	script_thread_t() :
		stack(nullptr), script_context(nullptr), _orig_owner(nullptr) {
...
public:
	...
	public:
		...
		bool operator==(const iterator &target) const {
			return _lis == target._lis && _li_ix == target._li_ix &&
				   _node == target._node && _fn_ix == target._fn_ix;
		}
...
public:
	...
	constexpr lazy_func_t(uint64_t hash, bool is_first_hash = true) :
		_f(nullptr),
		_h(is_first_hash ? 0 : hash),
		_1st_h(is_first_hash ? hash : 0) {}
...
public:
	...
	constexpr global_table_t(uint64_t **segments = nullptr) :
		_segments(segments) {}
...
struct ysc_header_t {
	...
	size_t code_page_size(size_t page_ix) const {
		return (page_ix == code_page_count() - 1) ? code_size % page_size_max
												  : page_size_max;
	}
	...
	void redirect() {
		...
		if (func_table) {
			for (size_t i = 0; i < imp_func_count; ++i) {
				imp_funcs[i] = reinterpret_cast<uintptr_t>(
					(*func_table)[_imp_func_hash(i)]);
			}
		} else {
			for (size_t i = 0; i < imp_func_count; ++i) {
...
public:
	template <typename T>
	T &as(ptrdiff_t offset = 0) {
		return rua::bit_as<T>(this, offset);
	}

	template <typename T>
	const T &as(ptrdiff_t offset = 0) const {
		return rua::bit_as<T>(this, offset);
	}
};

class entity_obj_t : public rua::bytes_hacker {
public:
	struct nav_t : public rua::bytes_hacker {
		const vector3 &pos() const {
			return as<vector3>(0x0050);
		}
...
public:
	...
	const acceleration_t &acceleration() const {
		return as<acceleration_t>(0x004C);
	}
...
struct entity_obj_map_t {
	...
	entity_obj_t *operator[](int handle) const {
		...
		if (ix >= size ||
			hanlde_remainders[ix] != static_cast<uint32_t>(handle) % 0x100) {
		...
		if (node_size != sizeof(node_t)) {
			...
			return reinterpret_cast<node_t *>(
					   reinterpret_cast<uintptr_t>(nodes) + ix * node_size)
				->value;
		}
