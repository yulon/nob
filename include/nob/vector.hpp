...
struct vector2 {
	...
	float distance(const vector2 &v2) const {
		return std::sqrt(std::pow(x - v2.x, 2) + std::pow(y - v2.y, 2));
	}
...
struct vector3 : public vector2 {
	...
	constexpr vector3(float x_val, float y_val = 0.0f, float z_val = 0.0f) :
		vector2(x_val, y_val), z(z_val) {}

	constexpr vector3(const vector2 &v2, float z_val = 0.0f) :
		vector2(v2), z(z_val) {}
	...
	vector3 fix_rot() const {
		return {fix_angle(x), fix_angle(y), fix_angle(z)};
	}

	float distance(const vector3 &v3) const {
		return std::sqrt(
			std::pow(x - v3.x, 2) + std::pow(y - v3.y, 2) +
			std::pow(z - v3.z, 2));
	}
