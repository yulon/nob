...
namespace nob { namespace arm {

hasher weapon_from_ammo_type(const hasher &amm_typ) {
	static const std::unordered_map<hasher, hasher> map{
		{"AMMO_PISTOL", "WEAPON_PISTOL"},
		{"AMMO_STUNGUN", "WEAPON_STUNGUN"},
		{"AMMO_FLAREGUN", "WEAPON_FLAREGUN"},
		...
		{"AMMO_RPG", "WEAPON_RPG"},
		{"AMMO_HOMINGLAUNCHER", "WEAPON_HOMINGLAUNCHER"},
		{"AMMO_FIREWORK", "WEAPON_FIREWORK"},
		{"AMMO_MINIGUN", "WEAPON_MINIGUN"},
		{"AMMO_GRENADELAUNCHER", "WEAPON_GRENADELAUNCHER"},
		...
		{"AMMO_PETROLCAN", "WEAPON_PETROLCAN"},
		{"AMMO_FIREEXTINGUISHER", "WEAPON_FIREEXTINGUISHER"},
...
const std::vector<hasher> &weapons_from_group(const hasher &group) {
	static const std::unordered_map<hasher, std::vector<hasher>> map =
		([]() -> std::unordered_map<hasher, std::vector<hasher>> {
...
#include "arm/display_info_map.inc"
	...
	if (it == map.end()) {
		return display_info_t{
			hr.src_str() && strlen(hr.src_str()) ? hr.src_str() : "???", "???"};
	}
	return it->second;
}
