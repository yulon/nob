...
namespace nob {
	std::vector<std::string> model::mp_vehicles;
	...
	namespace empc {
		...
		struct GlobalTable
		{
			...
			int64_t* AddressOf(int index) const{ return &GlobalBasePtr[index >> 18 & 0x3F][index & 0x3FFFF];}
		...
		int64_t(*GetModelInfo)(int, int64_t);//used to extract display names without actually calling natives
		...
		*/
		...
		void EnableCars::FindScriptAddresses()
		{
			...
			while(*(int64_t*)scriptTable == 0)
			{
				Sleep(100);
			}
			//DEBUGMSG("Found script base pointer %llX", (int64_t)scriptTable);
		...
		{
			int64_t patternAddr = FindPattern("\x4C\x8D\x05\x00\x00\x00\x00\x4D\x8B\x08\x4D\x85\xC9\x74\x11", "xxx????xxxxxxxx");
			if(!patternAddr)
			{
			...
			globalTable.GlobalBasePtr = (int64_t**)(patternAddr + *(int*)(patternAddr + 3) + 7);
			...
			GetModelInfo = (int64_t(*)(int, int64_t))(*(int*)(patternAddr - 0x12) + patternAddr - 0x12 + 0x4);
			displayNameOffset = *(int*)(patternAddr + 0x7);
			int64_t getHashKeyPattern = FindPattern("\x48\x8B\x0B\x33\xD2\xE8\x00\x00\x00\x00\x89\x03", "xxxxxx????xx");
			...
			//DEBUGMSG("Found global base pointer %llX", (int64_t)globalTable.GlobalBasePtr);
		}
		...
		void EnableCars::EnableCarsGlobal()
		{
			...
			{
				int64_t sigAddress = FindPattern("\x28\x26\xCE\x6B\x86\x39\x03", "xxxxxxx", (const char*)shopController->GetCodePageAddress(i), shopController->GetCodePageSize(i));
				if (!sigAddress)
				{
					continue;
				}
				//DEBUGMSG("Pattern Found in codepage %d at memory address %llX", i, sigAddress);
				int RealCodeOff = (int)(sigAddress - (int64_t)shopController->GetCodePageAddress(i) + (i << 14));
							...
							if ((*(int*)shopController->GetCodePositionAddress(funcOff + k) & 0xFFFFFF) == 0x01002E)
							{
								for (k = k + 1; k < 30; k++)
								{
									if (*(unsigned char*)shopController->GetCodePositionAddress(funcOff + k) == 0x5F)
									{
										...
										//DEBUGMSG("Found Global Variable %d, address = %llX", globalindex, (int64_t)globalTable.AddressOf(globalindex));
		...
		{
			int64_t curAddress = (int64_t)shopController->GetCodePositionAddress(scriptSwitchOffset + 2);
			...
			for (int i = 0; i<cases;i++)
			{
				...
				int64_t caseOff = (int64_t)shopController->GetCodePositionAddress(startOff + jumpoff);
				...
				model::mp_vehicles.push_back(displayName);
				HashPair h;
		...
		void EnableCars::PatchCheatController()
		{
			...
			int64_t StartOff = ((int64_t)(cheatController->localOffset + cheatController->localCount)+ 15) & 0xFFFFFFFFFFFFFFF0;
			//DEBUGMSG("Cheat controller Loaded, CodeLength = %d", cheatController->codeLength);

			int *setOffset = NULL;
			for (int i = 0; i < cheatController->codeLength;i++)
			{
				if (*(int64_t*)cheatController->GetCodePositionAddress(i) == 0x3C6E5C3C6E00002E)//searching for the address of code in the ysc where we will hook our new function
				{
			...
			unsigned char* cur = (unsigned char*)((int64_t)StartOff + 129);
	...
	void vehicle::unlock_mp_vehicles() {
		model::mp_vehicles.clear();
		empc::EnableCars::Run();
	}
} /* nob */
