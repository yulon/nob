...
void(WINAPI *scriptRegisterAdditionalThread)(
	HMODULE module, void(WINAPI *LP_SCRIPT_MAIN)());
...
constexpr const std::array<LPCWSTR, 2> _module_names{
	{L"ScriptHookV.dll", L"OpenVHook.dll"}};
...
#define GPA(_v, _n) _v = rua::generic_ptr(GetProcAddress(dll, _n))
		...
		GPA(scriptRegisterAdditionalThread,
		...
		GPA(getScriptHandleBaseAddress,
