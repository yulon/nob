...
void disable_sp_features(bool toggle) {
	...
	if (toggle) {
		if (!hk_bkr) {
			hk_bkr = {hotkey_t::MultiplayerInfo,
...
void disable_wheel_slowmo(bool toggle) {
	...
	if (toggle) {
		if (!sf) {
			sf = rua::wait([]() -> void (*)() {
				// Reference from
				// https://www.unknowncheats.me/forum/grand-theft-auto-v/181752-weapon-wheel-slowmotion.html
				auto mr = game_code.match(
					{0x48, 0x89, 0x5C, 0x24, 0x08, 0x57, 0x48, 0x83,
					 0xEC, 0x20, 0x33, 0xC0, 0x8B, 0xFA, 0x48, 0x8B,
					 0xD9, 0x83, 0xFA, 0x01, 0x75, 1111, 0x38, 0x05,
					 1111, 1111, 1111, 1111, 0x0F, 0x45, 0xF8});
				return mr.size() ? mr[0].data() : nullptr;
			});
			if (!sf) {
				return;
			}
...
bool menu::is_opened() const {
	return this->_data.get() && _menu::cur.get() &&
		   _menu::cur.get() == this->_data.get();
}
		...
		if (sz) {
			...
			if (sz < 11) {
				...
				g2d::draw_text(
					...
					y + ((h - font_height) / 2.0f),
					w,
					std::to_string(cur_li->selected + 1) + " / " +
						std::to_string(sz),
					font_size,
					255,
					255,
			...
			for (size_t i = cur_li->page_top; i < sz && i < end; ++i) {
				...
				if (i == cur_li->selected) {
					...
					if (cur_li->items[i].type_is<flag>()) {
						auto flg_val = cur_li->items[i].as<flag>()->value;
						if (flg_val) {
							_menu::cm_td.draw(
								"shop_box_tickb",
								x + w - icon_width,
								y + ((h - icon_height) / 2.0f),
								icon_width,
								icon_height);
						} else {
							_menu::cm_td.draw(
			...
			if (!cur_li->items[cur_li->selected]->desc.empty()) {
				...
				g2d::draw_text(
					x + item_padding_left,
					y + ((item_height - font_height) / 2),
					text_w,
					cur_li->items[cur_li->selected]->desc,
					font_size,
					255,
					255,
					255,
			...
			case hotkey_t::FrontendAccept: {
				...
				if (cur_li->items.size()) {
					...
					if (cur_it.type_is<action>()) {
						auto a = cur_it.as<action>();
						if (a->handler) {
							a->handler();
						}
					} else if (cur_it.type_is<list>()) {
						cur_li = cur_it.as<list>();
						if (cur_li->on_show) {
							cur_li->on_show(*cur_li);
							cur_li->fix();
						}
						_menu::cur->_li_stack.push(cur_li);
					} else if (cur_it.type_is<flag>()) {
						auto flg = cur_it.as<flag>();
						flg->value = !flg->value;
						if (flg->on_change) {
...
void banner(const std::string &title, const std::string &sub_title) {
	if (!_bnr::sf) {
		_bnr::sf =
			ntv::GRAPHICS::REQUEST_SCALEFORM_MOVIE("MP_BIG_MESSAGE_FREEMODE");
	...
	if (!_bnr::free) {
		_bnr::free = on_unload([]() {
			if (_bnr::sf &&
				ntv::GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(_bnr::sf)) {
				ntv::GRAPHICS::SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(
					&_bnr::sf);
			}
		});
	}
	...
	ntv::GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(
		_bnr::sf, "SHOW_SHARD_WASTED_MP_MESSAGE");
	...
	_bnr::draw_tsk = task([]() {
		ntv::GRAPHICS::DRAW_SCALEFORM_MOVIE_FULLSCREEN(
			_bnr::sf, 255, 255, 255, 255, 0);
	});
}
...
void button_bar(
	std::vector<std::pair<std::string, std::vector<hotkey_t>>> buttons) {
	if (!_btn_bar::sf) {
		_btn_bar::sf = ntv::GRAPHICS::REQUEST_SCALEFORM_MOVIE_INSTANCE(
		...
		ntv::GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(
			_btn_bar::sf, "TOGGLE_MOUSE_BUTTONS");
		ntv::GRAPHICS::_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_BOOL(false);
		ntv::GRAPHICS::END_SCALEFORM_MOVIE_METHOD();

		ntv::GRAPHICS::CALL_SCALEFORM_MOVIE_METHOD(
			_btn_bar::sf, "CREATE_CONTAINER");
	}
	if (!_btn_bar::reset) {
	...
	if (!_btn_bar::free) {
		_btn_bar::free = on_unload([]() {
			if (_btn_bar::sf &&
				ntv::GRAPHICS::HAS_SCALEFORM_MOVIE_LOADED(_btn_bar::sf)) {
				ntv::GRAPHICS::SET_SCALEFORM_MOVIE_AS_NO_LONGER_NEEDED(
					&_btn_bar::sf);
			}
		});
	...
	_btn_bar::draw_tsk = task([]() {
		for (auto i = 0; i < _btn_bar::data.size(); ++i) {
			auto &pr = _btn_bar::data[_btn_bar::data.size() - 1 - i];
			ntv::GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(
				_btn_bar::sf, "SET_DATA_SLOT");
			...
			for (auto it = pr.second.rbegin(); it != pr.second.rend(); ++it) {
				ntv::GRAPHICS::
					_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_BUTTON_NAME(
						ntv::CONTROLS::GET_CONTROL_INSTRUCTIONAL_BUTTON(
							0, static_cast<int>(*it), 0));
				// ntv::GRAPHICS::_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_BOOL(false);
				// ntv::GRAPHICS::_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(358);
			}
			ntv::GRAPHICS::_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING(
				pr.first.c_str());
			ntv::GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
		}
		ntv::GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(
			_btn_bar::sf, "DRAW_INSTRUCTIONAL_BUTTONS");
		ntv::GRAPHICS::_PUSH_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(-1);
		ntv::GRAPHICS::END_SCALEFORM_MOVIE_METHOD();

		ntv::GRAPHICS::DRAW_SCALEFORM_MOVIE_FULLSCREEN(
			_btn_bar::sf, 255, 255, 255, 255, 0);
	});
}
...
void enable_mp_frontend_menu(bool toggle) {
	...
	if (toggle) {
		...
		hk_lnr = hotkey_listener(
			{hotkey_t::FrontendPause, hotkey_t::FrontendPauseAlternate},
...
void stats_panel::reset(
	...
	ntv::GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(
		_sf, "SET_VEHICLE_INFOR_AND_STATS");
