...
namespace nob {
	...
	void _find_banned_vehicles(ntv::script_t *shop_ctrllr, size_t func_call_off) {
		for (size_t i = 14; i < 400; i++) {
			if (*(uint8_t *)shop_ctrllr->code_off_addr(func_call_off + i) == 0x62) {
				size_t switch_off = func_call_off + i;
				auto cur_addr = (uint64_t)shop_ctrllr->code_off_addr(switch_off + 2);
				...
				for (size_t i = 0; i < cases; i++) {
					...
					int64_t case_off = (int64_t)shop_ctrllr->code_off_addr(start_off + jump_off);
	...
	void vehicle::unlock_banned_vehicles() {
		...
		std::thread([shop_ctrllr, id_ch]() mutable {
			...
			size_t code_off = shop_ctrllr->code_off(addr);
			for (size_t j = 0; j < 2000; j++) {
				if (*(uint32_t *)shop_ctrllr->code_off_addr(code_off - j) == 0x0008012D) {
					size_t func_off = *(uint32_t *)shop_ctrllr->code_off_addr(code_off - j + 6) & 0xFFFFFF;
					for (size_t k = 0x5; k < 0x40; k++) {
						if ((*(uint32_t *)shop_ctrllr->code_off_addr(func_off + k) & 0xFFFFFF) == 0x01002E) {
							for (k = k + 1; k < 30; k++) {
								if (*(uint8_t *)shop_ctrllr->code_off_addr(func_off + k) == 0x5F) {
									_find_banned_vehicles(shop_ctrllr, code_off - j);
									id_ch << (*(uint32_t *)shop_ctrllr->code_off_addr(func_off + k + 1) & 0xFFFFFF);
									return;
								}
							}
							break;
