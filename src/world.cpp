...
void no_mans_island(bool toggle) {
	...
	if (toggle) {
		...
		tsk = task([]() {
			...
			ntv::VEHICLE::SET_RANDOM_VEHICLE_DENSITY_MULTIPLIER_THIS_FRAME(
				0.0f);
			ntv::VEHICLE::SET_PARKED_VEHICLE_DENSITY_MULTIPLIER_THIS_FRAME(
				0.0f);
			...
			ntv::PED::SET_SCENARIO_PED_DENSITY_MULTIPLIER_THIS_FRAME(
				0.0f, 0.0f);
		...
		ntv::GAMEPLAY::_CLEAR_AREA_OF_EVERYTHING(
			pos.x, pos.y, pos.z, 1000, false, false, false, false);
		...
		auto h = ntv::PED::ADD_SCENARIO_BLOCKING_AREA(
			-7000.0f, -7000.0f, -100.0f, 7000.0f, 7000.0f, 315.0f, 0, 1, 1, 1);
		ou =
			on_unload([h]() { ntv::PED::REMOVE_SCENARIO_BLOCKING_AREA(h, 0); });
...
vector3 load_ilp(ilp i, bool toggle) {
	...
	case ilp::bahama_mamas:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					-1388.0013427734375f,
					-618.419677734375f,
					30.819599151611328f),
				false);
	...
	case ilp::chopshop: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			479.0567932128906f, -1316.8253173828125f, 28.2038f);
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(-664582244, 482.8112f,
			// -1311.953f, 29.35057f, true, 0.0f, 0.0f, -1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-190780785, 484.5652f,
			// -1315.574f, 30.20331f, true, 0.0f, 0.0f, 0.0f);
		} else {
	...
	case ilp::fib_lobby:
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(-1517873911, 106.3793f,
			// -742.6982f, 46.51962f, false, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-90456267, 105.7607f,
			// -746.646f, 46.18266f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::STREAMING::REQUEST_IPL("FIBlobbyfake");
			ntv::STREAMING::REMOVE_IPL("FIBlobby");
			// ntv::OBJECT::_DOOR_CONTROL(-1517873911, 106.3793f,
			// -742.6982f, 46.51962f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-90456267, 105.7607f,
			// -746.646f, 46.18266f, true, 0.0f, 0.0f, 0.0f);
		}
		return {105.7607f, -746.646f, 46.18266f};

	case ilp::foundry: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			1087.1951904296875f, -1988.4449462890625f, 28.649f);
		if (toggle) {
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_foundry");
			// ntv::OBJECT::_DOOR_CONTROL(-1428622127, 1083.547f,
			// -1975.435f, 31.62222f, true, 0.0f, 0.0f, -1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1428622127, 1085.307f,
			// -2018.561f, 41.62894f, true, 0.0f, 0.0f, 1.0f);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
	...
	case ilp::floyds_house:
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(-607040053, -1149.709f,
			// -1521.088f, 10.78267f, false, 0.0f, 0.0f, 0.0f);
		}
		return {-1149.709f, -1521.088f, 10.78267f};

	case ilp::epsilon: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			245.15640258789062f, 370.21099853515625f, 104.73819732666016f);
		if (toggle) {
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_epsilonism");
			// ntv::OBJECT::_DOOR_CONTROL(-1230442770, 241.3621f, 361.0471f,
			// 105.8883f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::INTERIOR::DISABLE_INTERIOR(intr, true);
			ntv::STREAMING::REMOVE_IPL("v_epsilonism");
			// ntv::OBJECT::_DOOR_CONTROL(-1230442770, 241.3621f, 361.0471f,
			// 105.8883f, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::hospital_destroyed:
		if (toggle) {
			...
			ntv::STREAMING::REQUEST_IPL(
				"RC12B_HospitalInterior"); // state 3 2of2 Destroyed Interior
		}
		return {0, 0, 0};
	...
	case ilp::janitor:
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(486670049, -107.5373f,
			// -9.018099f, 70.67085f, false, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::jewel_store:
		if (toggle) {
			auto intr =
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(-630.4f, -236.7f, 40.0f);
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::UNPIN_INTERIOR(intr);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			...
			// ntv::OBJECT::_DOOR_CONTROL(9467943, -630.4265f,
			// -238.4375f, 38.20653f, true, 0.0f, 0.0f, 1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(1425919976, -631.9554f,
			// -236.3333f, 38.20653f, true, 0.0f, 0.0f, -1.0f);
		}
	...
	case ilp::lab:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					3522.84521484375f, 3707.96533203125f, 19.9918f),
				false);
			ntv::STREAMING::REQUEST_IPL("v_lab");
			// ntv::OBJECT::_DOOR_CONTROL(-1081024910, 3620.843f,
			// 3751.527f, 27.69009f, false, 0.0f, 0.0f, -1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1081024910, 3627.713f,
			// 3746.716f, 27.69009f, false, 0.0f, 0.0f, -1.0f); elevator coords
			// 3540.65f, 3675.77f, 28.12f elevator2 coords 3540.65f,
			// 3675.77f, 20.12f
		}
	...
	case ilp::life_invader:
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(-340230128, -1042.518,
			// -240.6915, 38.11796, true, 0.0f, 0.0f, -1.0f);
		} else {
			ntv::STREAMING::REMOVE_IPL("facelobby"); // lifeinvader
			ntv::STREAMING::REQUEST_IPL("facelobbyfake");
			// ntv::OBJECT::_DOOR_CONTROL(-340230128, -1042.518,
			// -240.6915, 38.11796, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::Lesters_house:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					1274.933837890625f, -1714.7255859375f, 53.77149963378906f),
				false);
			ntv::STREAMING::REQUEST_IPL("v_lesters");
			// ntv::OBJECT::_DOOR_CONTROL(1145337974, 1273.816f,
			// -1720.697f, 54.92143f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					1274.933837890625f, -1714.7255859375f, 53.77149963378906f),
			...
			// ntv::OBJECT::_DOOR_CONTROL(1145337974, 1273.816f,
			// -1720.697f, 54.92143f, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::lesters_factory:
		if (toggle) {
			...
			// ntv::OBJECT::_DOOR_CONTROL(826072862, 717.0f, -975.0f, 25.0f,
			// false, 0.0f, 0.0f, 0.0f); ntv::OBJECT::_DOOR_CONTROL(763780711,
			// 719.0f, -975.0f, 25.0f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::STREAMING::REMOVE_IPL("id2_14_during_door");
			ntv::STREAMING::REMOVE_IPL("id2_14_during1");
			ntv::STREAMING::REMOVE_IPL("id2_14_during2");
	...
	case ilp::ranch: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			1399.9730224609375f, 1148.755859375f, 113.33360290527344f);
		if (toggle) {
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::INTERIOR::_LOAD_INTERIOR(intr);
			ntv::INTERIOR::UNPIN_INTERIOR(intr);
			////ntv::OBJECT::_DOOR_CONTROL(1504256620, 1395.92f, 1142.904f,
			/// 114.7902f, true, 0.0f, 0.0f, 1.0f);
			////ntv::OBJECT::_DOOR_CONTROL(-52575179, 1390.666f, 1133.317f,
			/// 114.4808f, true, 0.0f, 0.0f, -1.0f);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
		}
	...
	case ilp::rogers_salvage_and_scrap: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			-598.6378784179688f, -1608.3985595703125f, 26.010799407958984f);
		if (toggle) {
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::UNPIN_INTERIOR(intr);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_recycle");
			// ntv::OBJECT::_DOOR_CONTROL(1099436502, -608.7289f,
			// -1610.315f, 27.15894f, false, 0.0f, 0.0f, -1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1627599682, -611.32f,
			// -1610.089f, 27.15894f, false, 0.0f, 0.0f, 1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(1099436502, -592.9376f,
			// -1631.577f, 27.15931f, false, 0.0f, 0.0f, -1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1627599682, -592.7109f,
			// -1628.986f, 27.15931f, false, 0.0f, 0.0f, 1.0f);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
			ntv::STREAMING::REMOVE_IPL("v_recycle");
			// ntv::OBJECT::_DOOR_CONTROL(1099436502, -608.7289f,
			// -1610.315f, 27.15894f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1627599682, -611.32f,
			// -1610.089f, 27.15894f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(1099436502, -592.9376f,
			// -1631.577f, 27.15931f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1627599682, -592.7109f,
			// -1628.986f, 27.15931f, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::sheriff_office_paleto:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					-444.89068603515625f, 6013.5869140625f, 30.7164f),
				false);
			ntv::INTERIOR::CAP_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					-444.89068603515625f, 6013.5869140625f, 30.7164f),
				false);
			ntv::STREAMING::REQUEST_IPL("v_sheriff2");
			ntv::STREAMING::REMOVE_IPL("cs1_16_sheriff_cap");
			// ntv::OBJECT::_DOOR_CONTROL(-1501157055, -444.4985f,
			// 6017.06f, 31.86633f, false, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1501157055, -442.66f,
			// 6015.222f, 31.86633f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					-444.89068603515625f, 6013.5869140625f, 30.7164f),
			...
			// ntv::OBJECT::_DOOR_CONTROL(-1501157055, -444.4985f,
			// 6017.06f, 31.86633f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(-1501157055, -442.66f,
			// 6015.222f, 31.86633f, true, 0.0f, 0.0f, 0.0f);
		}
		return {-442.66f, 6015.222f, 31.86633f};

	case ilp::sheriff_office_sandy_shores: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			1854.2537841796875f, 3686.738525390625f, 33.2671012878418f);
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_sheriff");
			ntv::STREAMING::REMOVE_IPL("sheriff_cap");
			// ntv::OBJECT::_DOOR_CONTROL(-1765048490, 1855.685f,
			// 3683.93f, 34.59282f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
			ntv::STREAMING::REMOVE_IPL("v_sheriff");
			ntv::STREAMING::REQUEST_IPL("sheriff_cap");
			// ntv::OBJECT::_DOOR_CONTROL(-1765048490, 1855.685f,
			// 3683.93f, 34.59282f, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::slaughter_house: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			982.2329711914062f, -2160.381591796875f, 28.4761f);
		if (toggle) {
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_abattoir");
			// ntv::OBJECT::_DOOR_CONTROL(-1468417022, 962.9084f,
			// -2105.813f, 32.52716f, true, 0.0f, 0.0f, 1.0f);
			// ntv::OBJECT::_DOOR_CONTROL(1755793225, 962.0066f,
			// -2183.816f, 31.06194f, true, 0.0f, 0.0f, 1.0f);
			ntv::INTERIOR::UNPIN_INTERIOR(intr);
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
			// ntv::OBJECT::_DOOR_CONTROL(-1468417022, 962.9084f,
			// -2105.813f, 32.52716f, true, 0.0f, 0.0f, 0.0f);
			// ntv::OBJECT::_DOOR_CONTROL(1755793225, 962.0066f,
			// -2183.816f, 31.06194f, true, 0.0f, 0.0f, 0.0f);
	...
	case ilp::solomons_office:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					-1005.663208f, -478.3460998535156f, 49.0265f),
				false);
	...
	case ilp::trevors_trailer:
		if (toggle) {
			...
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(1973.0f, 3815.0f, 34.0f),
				false);
			ntv::STREAMING::REQUEST_IPL("TrevorsTrailerTidy");
			// ntv::OBJECT::_DOOR_CONTROL(132154435, 1972.769f,
			// 3815.366f, 33.66326f, false, 0.0f, 0.0f, 0.0f);
		}
		return {1972.769f, 3815.366f, 33.66326f};

	case ilp::tequl_la_la: {
		auto intr = ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
			-556.5089111328125f, 286.318115234375f, 81.1763f);
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(intr, false);
			ntv::INTERIOR::CAP_INTERIOR(intr, false);
			ntv::STREAMING::REQUEST_IPL("v_rockclub");
			// ntv::OBJECT::_DOOR_CONTROL(993120320, -565.1712f,
			// 276.6259f, 83.28626f, false, 0.0f, 0.0f, 0.0f);// front door
			// ntv::OBJECT::_DOOR_CONTROL(993120320, -561.2866f,
			// 293.5044f, 87.77851f, false, 0.0f, 0.0f, 0.0f);// back door
		} else {
			ntv::INTERIOR::CAP_INTERIOR(intr, true);
			ntv::STREAMING::REMOVE_IPL("v_rockclub");
			// ntv::OBJECT::_DOOR_CONTROL(993120320, -565.1712f,
			// 276.6259f, 83.28626f, true, 0.0f, 0.0f, 0.0f);// front door
			// ntv::OBJECT::_DOOR_CONTROL(993120320, -561.2866f,
			// 293.5044f, 87.77851f, true, 0.0f, 0.0f, 0.0f);// back door
		}
	...
	case ilp::torture:
		if (toggle) {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					147.43299865722656f, -2201.370361328125f, 3.688f),
				false);
			ntv::STREAMING::REQUEST_IPL("v_torture");
			// ntv::OBJECT::_DOOR_CONTROL(464151082, 134.3954f,
			// -2204.097f, 7.514325f, false, 0.0f, 0.0f, 0.0f);
		} else {
			ntv::INTERIOR::DISABLE_INTERIOR(
				ntv::INTERIOR::GET_INTERIOR_AT_COORDS(
					147.43299865722656f, -2201.370361328125f, 3.688f),
			...
			// ntv::OBJECT::_DOOR_CONTROL(464151082, 134.3954f,
			// -2204.097f, 7.514325f, true, 0.0f, 0.0f, 0.0f);
		}
	...
	case ilp::mp_yacht:
		if (toggle) {
			...
			ntv::AUDIO::SET_AMBIENT_ZONE_LIST_STATE_PERSISTENT(
				"AZL_HEI_AIRCRAFT_CARRIER", true, true);
		}
	...
	case ilp::mp_heist_carrier:
		if (toggle) {
			...
			ntv::AUDIO::SET_AMBIENT_ZONE_LIST_STATE_PERSISTENT(
				"AZL_DLC_HEI_MILO_YACHT_ZONES", true, true);
		}
...
void snowy(bool toggle) {
	...
	static uint8_t *block_code_addr = rua::wait([]() -> uint8_t * {
		// Reference from
		// http://gtaforums.com/topic/902339-enable-snowy-map-in-single-player/
		auto mr = game_code.match({0x74,
								   0x25,
								   0xB9,
								   0x40,
								   0x00,
